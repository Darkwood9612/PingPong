...
Window::Window()
{
	...
	this->playerScoreRect = { int(SCREEN_WIDTH * X_PLAYER_SCORE_OFFSET), int(SCREEN_HEIGHT * Y_SCORE_OFFSET),0,0 };
	this->botScoreRect = { int(SCREEN_WIDTH * X_BOT_SCORE_OFFSET), int(SCREEN_HEIGHT * Y_SCORE_OFFSET),0,0 };
	this->dividingStripRect = { SCREEN_WIDTH / 2,0,0,0 };
}

Window::Window(const char* title, uint32_t windowFlags, const int _SCREEN_WIDTH, const int _SCREEN_HEIGHT) :
